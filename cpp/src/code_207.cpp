...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        auto dep_graph = create_graph(static_cast<std::size_t>(numCourses), 
                                    prerequisites);
...
private:
    ...
    CourseGraph create_graph(std::size_t numCourses, 
                           const std::vector<std::vector<int>>& prereq_list) {
    ...
    bool resolve_dependencies(int course, CourseGraph& dep_graph) {
        if (dep_graph[course].state == NodeState::COMPLETED) {
            return true;
        }

        if (dep_graph[course].state == NodeState::DISCOVERED) {
            return false;
        }
