...
test_cases.forEach((test_case) => {
    ...
    if (Object.hasOwn(test_case.output, "rejected")) {
        expect(limited(test_case.input.inputs)).to.be.rejectedWith(
            test_case.output.rejected
        );
    } else {
        expect(limited(...test_case.input.inputs)).to.eventually.equal(
            test_case.output.resolved
        );
    }
});
